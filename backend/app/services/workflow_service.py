...
def _run_workflow_sync(
    ...
    runner = WorkflowRunner(
        ...
        timeframe=timeframe,
        databases=databases,
        db_callbacks={"node_completed": notify_node_completed},
    )
...
async def _persist_node_completed(
    ...
    """Persist intermediate node progress immediately so active users can leave and return."""
    from app.workflow.runner import _serialize_state_snapshot
    try:
        safe_output = _serialize_state_snapshot(output_summary) if output_summary else {}
        async with AsyncSessionLocal() as db:
            # Record the technical node execution
        ...
        # Update the live state of the research so status bars are correct
        result = await db.execute(
            select(ResearchHistory).where(ResearchHistory.id == uuid.UUID(research_id))
        )
        research = result.scalar_one_or_none()
        ...
        # Post a timeline event message to render historically when page reloads
        ...
        msg = ResearchMessage(
            ...
            content=msg_content,
            message_type="node_event",
            metadata_extra={"duration_ms": duration_ms, "output": output_summary}
        )
        db.add(msg)
